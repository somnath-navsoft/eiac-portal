...
@Component({
  selector: 'app-candidate-trainer-service',
  templateUrl: './candidate-trainer-service.component.html',
  styleUrls: ['./candidate-trainer-service.component.scss']
})
export class CandidateTrainerServiceComponent implements OnInit {
...
export class CandidateTrainerServiceComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    //     {
    //     title:'course', desc:'1. Course <br> Information', activeStep:true, stepComp:false, active:'user-done', nextStep:'payment'
    //     },
    //     {
    //     title:'payment', desc:'2. Payment <br> Information', activeStep:false, stepComp:false, active:'', nextStep:'review'
    //     },
    //     {
    //     title:'review', desc:'3. Review <br> Information', activeStep:false, stepComp:false, active:'', nextStep:'finish'
    //     },
    //     {
    //     title:'finish', desc:'4. Finish', activeStep:false, stepComp:false, active:'', nextStep:null
    //     }
    // );
    this._route.params.subscribe(params => {
        ...
        if(params.id != undefined && params.id > 0){
          ...
          this.headerSteps.push(
            ...
            {
            title:'payment', desc:'2. Payment <br> Information', activeStep:false, stepComp:false, active:'', nextStep:null
            }
          );
        }
    });
  ...
  headerStepClick(stepId: string, target?: string){
     ...
     //search active steps
     let activeCurr = this.headerSteps.find(rec => rec.activeStep == true);
     //console.log("cur active: ", activeCurr, " -- ", this.headerSteps);
     if(activeCurr){
      //targetElem =  document.getElementById(activeCurr.title);
     ...
     if(curStep > 0 || curStep == 0){
       ...
       if(curStep-1 < 0){
        ...
        var prevSteps = this.headerSteps[curStep-1];
  ...
  onPublicCourse(){
     ...
     if(this.isValidPublicCourse()){
        ...
        postObject['agreement_status'] = (this.isAgree) ? 'accepted' : 'rejected';
            ...
            .subscribe(
              result => { 
               ...
               if(data && data.status == 200 && data.response === 'success'){   
                 ...
                 this._router.navigateByUrl('/dashboard/candidate/training-service');
               }
             }
          ))
        }
  ...
  onCourseStepNext(){
      ...
      if(findStep){
        ...
        this.headerSteps[findStep].activeStep = true;
  ...
  onPaymentNext(){ 
    this._service.moveSteps('payment','review',this.headerSteps);
      /*
      //console.log("Payment...");
      let curStep = this.headerSteps.find(rec => rec.title === 'payment');
      let findStep = this.headerSteps.findIndex(rec => rec.title === 'review');
  ...
  onReviewNext(){
    this._service.moveSteps('review','finish',this.headerSteps);
     /*
     //console.log("Review...");
      let curStep = this.headerSteps.find(rec => rec.title === 'review');
      let findStep = this.headerSteps.findIndex(rec => rec.title === 'finish');
      //console.log("find stete: ", findStep);
      if(findStep){
        this.headerSteps[findStep].active = 'user-done';
        //previous steps;
