...
@Component({
  selector: 'app-trainers-event-list',
  templateUrl: './trainers-event-list.component.html',
  styleUrls: ['./trainers-event-list.component.scss']
})
export class TrainersEventListComponent implements OnInit {
...
import { Listing, ListingEvent, Delete } from '../../../../store/actions/trainer.actions';
...
export class TrainersEventListComponent implements OnInit, OnDestroy {
  ...
  constructor(private store: Store<TrainerState>, private _service: AppService, private _constant: Constants,
    ...
    this.store.dispatch(new ListingEvent({}));
    this.modalOptions = {
      backdrop:'static',
  ...
  filterSearchSubmit(){
     ...
     if(this.isValidSearch()){
       ...
       if(this.selectEndDate != null && this.selectEndDate != ''){
        ...
        //console.log(">>>POST: ", postObject, " -- ", postObject['end_date'], " -- ", postObject['start_date']);
        ...
        if((postObject['start_date'] != '' && postObject['start_date'] != undefined) && 
          ...
          //const daysDiff = Math.floor(Math.abs(<any>endDate - <any>stDate) / (1000*60*60*24));
        ...
        if(postObject){
          this.subscriptions.push(this._trainerService.searchEvent((postObject))
          .subscribe(
             result => {
               let data: any = result;
